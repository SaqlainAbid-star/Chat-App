...
firebase.database().ref('messages2').on('child_added',function (data){
    ...
    li.appendChild(liText)
     ...
     delBtn2.setAttribute("class","btn btn-primary rounded-pill btn-sm ml-2")
...
function send(){
    ...
    var key = firebase.database().ref('messages2').push().key
    ...
    firebase.database().ref('messages2').child(key).set(message2)
...
firebase.database().ref('messages1').on('child_added',function (data){
    ...
    li.appendChild(liText)
     ...
     delBtn1.setAttribute("class","btn btn-success rounded-pill btn-sm ml-2")
